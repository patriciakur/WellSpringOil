...
async function updateUser(userID, fullname, add1, add2, city, state, zip, info_complete) {
...
router.get('/profileManagement/:userID', async(req, res) =>{
    const username = req.params.userID
    console.log(username + " accessed profile management"); 

});  

router.post('/profileManagement/:userID', async(req, res) => {
    ...
    if (fname && taddress1 && zip && city && state) { // Check if all required fields are present
        ...
        await updateUser(curr_user.userID, curr_user.fname, curr_user.add, curr_user.add2, curr_user.city, curr_user.state, curr_user.zip, curr_user.info_completed, curr_user.username);
